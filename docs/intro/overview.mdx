...
With **`sync --with-github`** (and credentials), hyper-pm **pushes** ticket changes to GitHub Issues, **pulls** inbound edits, and can attach **PR timeline** signals for eligible tickets with linked PR references (not `done` / `cancelled`). Opening a PR can record **`GithubPrActivity`** and move the ticket to **`in_progress`**. Plain **`sync`** is git-only. See [GitHub sync](/integrations/github-sync) for details.
